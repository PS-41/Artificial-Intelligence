...
np.random.seed(41)
tp = [0, 1, 2, 4, 6, 7, 8, 9, 12, 13, 14, 16, 19, 24, 25, 26, 28, 29, 31, 33]
...
test_data = pd.read_csv('Camera Roll.csv')
test_col_names = list(test_data.columns.values)
test_data = test_data.drop(test_col_names[: 679], axis = 1) # Dropping all non AU columns
test_col_names = list(test_data.columns.values)
test_dimensions = len(test_col_names)
X = test_data.iloc[:, :test_dimensions].values
X_selected_features = X[:, tp]
...
# print(test_col_names, len(test_col_names))
